...
class MainWindow(QWidget):
    def __init__(self):
        ...
        self.year.addItems(['1991','2001','2011'])
        self.year.setCurrentText('2011')
        ...
        self.states.activated.connect(self.onIndicatorSelect)
        self.year.activated.connect(self.onIndicatorSelect) # TODO
    ...
    def onIndicatorSelect(self):
        ...
        if state=='India' and int(year)<2018: # for 2021 census we will use latest 2019 map
            mapURL = 'https://besnoi.github.io/maps/json/india2013.json'
        else:
        ...
        str(data)+
        "; Highcharts.mapChart('container',{chart:{map: topology}, title:{text: '"+indicator[:-4]+" in "+state+" (%)'}, subtitle:{text: '"+state+": "+str(data[0][1])+"',align: 'center',verticalAlign: 'bottom'}, legend:{layout: 'vertical', align: 'right', verticalAlign: 'middle'},mapNavigation:{enabled: true, buttonOptions:{verticalAlign: 'bottom'}}, colorAxis:{align: 'right',verticalMiddle:'middle',min: 0, maxColor: '"+maxColor+"'}, credits:{enabled:false}, series: [{data: data, name: '"+number+"', states:{hover:{color: '#BADA55'}}, dataLabels:{enabled: true, format: '{point.name}'}}]});})();</script>")
        # print(html)
